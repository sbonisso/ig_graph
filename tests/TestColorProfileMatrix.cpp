...
TEST_CASE("test ColorProfileMatrix",
	  "[test1]") {
	...
	for(int i = 0; i < (int)v_truth.size(); i++) {
	    CHECK(vscore[i] == v_truth[i]); //, "V scores are incorrect");
	}
	for(int i = 0; i < (int)d_truth.size(); i++) {
	    CHECK(dscore[i] == d_truth[i]); //, "D scores are incorrect");
	}
	for(int i = 0; i < (int)j_truth.size(); i++) {
	    CHECK(jscore[i] == j_truth[i]); //, "J scores are incorrect");
	}
    }

    SECTION("cp_matrix_scores_2") {
	...
	CHECK(j_part[2].first == 322); //, "J start is incorrect!");
	CHECK(j_part[2].second == 371); //, "J end is incorrect!");
    }
