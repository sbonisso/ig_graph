...
pair<string,string> FastaParser::getNextEntry() {
    ...
    do {
	// placed before getline for last line - so that don't duplicate
	if(this->fin.eof()) { currEntry.second = str; return currEntry; }
...
string FastaParser::getID() {
    // take substr up to first space, cut off leading '>' 
    istringstream iss(currIDLine);
    string stest;
    getline(iss, stest, ' ');    
    return (stest.size() > 1 ? 
	    (stest[0] == '>' ? stest.substr(1,stest.size()) : stest) 
	    : "");
}
